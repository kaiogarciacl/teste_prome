...
md.comando = f""" update login set ATIVO = case when ISNULL(DATEDIFF(Mi,TEMPO,GETDATE()),0) > '5' and TEMPO is not null then 'DESLOGADO' else 'LOGADO' end 
update login set ATIVO = case when TEMPO is null then '' else ATIVO end"""
cn.ins_upd_del()
...
if md.liberador_excucao == 'Não Liberado':
    st.toast('carregando')
...
with st.sidebar:
    ...
    if log != '':
        rows = f""" select ATIVO from login where nome = '{log}' """
    ...
    if log != '' and senha != '':
        #st.toast('**⚠️ Atenção as 10:00 H o sistema Será Reiniciado Voltando as 10:15 H**')
        md.usuario = log
        ...
        for row in rows:
            ...
            if verificador_log == 'NÃO CONSTA':
                ...
                if variavel_log == 'LOGADO':
                    deslogar = st.button('Deslogar', use_container_width=True, on_click=limpa_log)
                    md.usuario = log
if log != '' and senha != '':
    if variavel_log == 'DESLOGADO' and verificador_log == 'CONSTA':
        modal = Modal(f"Seção Desconectada", key="demo-modal", padding=20, max_width=250)
        with modal.container():
            def reconectar():
                md.comando = f""" update login set TEMPO = GETDATE(), ATIVO = 'LOGADO' where nome = '{md.usuario}'"""
                cn.ins_upd_del()
            st.button('Reconectar', use_container_width=True, on_click=reconectar)

if log == '' or senha == '':
    apontamento.apontamento()

if log != '' and senha != '' and verificador_log == 'CONSTA' and variavel_log == 'LOGADO':
    md.comando = f""" update login set TEMPO = GETDATE(), ATIVO = 'LOGADO' where nome = '{md.usuario}'"""
    cn.ins_upd_del()
    ...
    if opcao == 'Previsão de Compras':
        previsao_chapa.previsao()

    if opcao == 'Indicadores':
        indicador_ppm.indicador_ppm()

    if opcao == 'Solicitar Orçamento':
        solicitar_orcamento.solicitacao_orcamento()
